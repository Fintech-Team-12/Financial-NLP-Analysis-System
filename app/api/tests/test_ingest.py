...
def test_ingest_valid_year(client: TestClient) -> None:
    ...
    response = client.post("/api/reports/reports/ingest", json={"year": 2023})
...
def test_ingest_creates_documents(client: TestClient) -> None:
    """인덱싱 후 /api/health/health 에서 문서 수가 증가해야 한다."""
    client.post("/api/reports/reports/ingest", json={"year": 2023})
    health = client.get("/api/health/health").json()
...
def test_ingest_missing_year(client: TestClient) -> None:
    ...
    response = client.post("/api/reports/reports/ingest", json={"year": 2025})
...
def test_ingest_force_reindex(client: TestClient) -> None:
    ...
    client.post("/api/reports/reports/ingest", json={"year": 2023})
    first_count = client.get("/api/health/health").json()["total_documents"]

    response = client.post("/api/reports/reports/ingest", json={"year": 2023, "force": True})
    assert response.status_code == 200
    assert response.json()["indexed_count"] > 0
    # 재인덱싱 후 문서 수는 동일해야 함 (중복 없이 교체)
    second_count = client.get("/api/health/health").json()["total_documents"]
    assert second_count == first_count


def test_ingest_response_schema(client: TestClient) -> None:
    """응답에 year, indexed_count, message 필드가 있어야 한다."""
    data = client.post("/api/reports/reports/ingest", json={"year": 2023}).json()
    assert "year" in data
